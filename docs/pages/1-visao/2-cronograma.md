# Cronograma e Entregas

Consultando os códigos das User Stories da seção [Backlog de Produto](./5-backlog.md), o cronograma preliminar do projeto pode ser visualizado logo abaixo:

!!! warning

    O cronograma **poderá sofrer alterações** ao longo do desenvolvimento. </br>
    Não se preocupe, toda mudança será alertada antecipadamente.

| Sprint | Início | Fim | Objetivo de Ciclo | Entregas Previstas | Validação com Stakeholder |
| ------ | ------ | --- | ----------------- | ------------------ | ------------------------- |
| Sprint 1 | | | | | |
| Sprint 2 | | | | | |
| Sprint 3 | | | | | |
| Sprint 4 | | | | | |
| Sprint 5 | | | | | |
| Sprint 6 | | | | | |
| Sprint 7 | | | | | |
| Sprint 8 | | | | | |
| Sprint 9 | | | | | |
| Sprint 10 | | | | | |

</br>
</br>

---
## Histórico de Versão da Página

::timeline::

- title: v1.0
  sub_title: 28/08/2026
  content: Criação do documento por [Daniel Rodrigues](https://github.com/DanielRogs).
  icon: ':material-file-document-plus-outline:'

::/timeline::